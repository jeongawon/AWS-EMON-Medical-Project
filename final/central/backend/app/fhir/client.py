...
async def update(resource_type: str, resource_id: str, body: dict) -> dict:
    """PUT a FHIR resource — replace existing in place (no new version chain branch)."""
    client = await _get_client()
    body = {**body, "id": resource_id}  # FHIR PUT requires id in body to match URL
...
async def patch(resource_type: str, resource_id: str, body: dict) -> dict:
    """PATCH a FHIR resource using JSON Patch (RFC 6902).

    HAPI FHIR는 기본적으로 application/merge-patch+json을 거부하므로
    body의 각 key를 JSON Patch op로 변환하여 전송한다.
    예: {"status": "active"} → [{"op":"replace","path":"/status","value":"active"}]
    """
    client = await _get_client()
    ops = [{"op": "replace", "path": f"/{k}", "value": v} for k, v in body.items()]

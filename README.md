...
> EC2 실제 추론 결과 기준 (study_id → S3 파형 매칭 검증 완료)
> 108개 ECG 추론 대상 중 시나리오별 대표 케이스 자동 선별

### 시나리오 구성 의도

| 유형 | 케이스 | 시연 포인트 |
|------|--------|-----------|
| ECG 단독 확정 | Case 1, 2 | ECG 파형만으로 질환 확진 — 학습 잘 된 핵심 질환 |
| ECG 한계 → Lab 필수 | Case 3, 4 | ECG가 의심하지만 확정은 혈액검사가 해야 하는 질환 |
| ECG 경고 → 멀티모달 확정 | Case 5 | ECG가 전신 위험 감지 → Blood + Chest 모달 호출 |

---

### Case 1: ECG 확정 — 심방세동 94.0% + 심부전 70.4%

> **"ECG 파형에 f파와 불규칙 RR 간격이 명확합니다. 모델이 94% 신뢰도로 심방세동, 70%로 심부전을 동시 검출했습니다."**

| 항목 | 내용 |
|------|------|
| 환자 | 78세 남성 (subject_id: 18161880, study_id: 40985856) |
| 주소 | 대동맥 협착, Afib, CAD, CKD, DM |
| 실제 진단 (MIMIC-IV 퇴원요약) | Aortic Stenosis, **Atrial Fibrillation**, **CHF systolic**, CAD, CKD, DM |

**EC2 실제 결과:**

| 감지 질환 | 신뢰도 | 중증도 | 실제 진단 매칭 | 판정 |
|----------|--------|--------|-------------|------|
| **심방세동/조동** | **94.0%** | MODERATE | ✅ Atrial Fibrillation | **TP** |
| **심부전** | **70.4%** | MODERATE | ✅ CHF systolic (EF 35%) | **TP** |
| 심방세동(세부) | 89.5% | MODERATE | ✅ | TP |
| 심부전(세부) | 68.4% | MODERATE | ✅ | TP |

- ECG Vitals: **irregular_rhythm=true** → Afib 파형과 독립적으로 일치
- Risk Level: **URGENT**
- 오탐: **0개** / 미탐: CAD(39.2%, threshold 이하), CKD(44.8%, threshold 이하)

**시연 발표:**

> "78세 남성 환자입니다. ECG AI 분석 결과 심방세동 94%, 심부전 70%가 검출되었습니다.
> 실제 MIMIC-IV 퇴원요약에도 Atrial Fibrillation과 CHF systolic이 기록되어 있어 **True Positive**입니다.
> ECG Vitals에서도 irregular_rhythm=true로 모델과 독립적으로 부정맥을 확인했습니다.
> 심방세동은 f파와 불규칙 RR 간격이라는 **ECG 특이 소견**이 있어 ECG만으로 확정 가능한 대표적 질환입니다.
> 오탐 0건으로 **학습이 가장 잘 된 케이스**입니다."

---

### Case 2: ECG 확정 — 심부전 74.1% (Golden Dx Primary 일치)

> **"Golden Dx Primary가 '급성 악화 이완기 심부전'인 환자에서 모델이 심부전을 74.1%로 정확히 잡았습니다."**

| 항목 | 내용 |
|------|------|
| 환자 | 88세 남성 (subject_id: 10299107, study_id: 42536764) |
| 주소 | HFpEF (TTR 아밀로이드증), 발작성 심방세동, 고혈압, 호흡곤란 |
| 실제 진단 (MIMIC-IV 퇴원요약) | **Acute on chronic diastolic heart failure** (Primary), Afib with RVR, HTN |

**EC2 실제 결과:**

| 감지 질환 | 신뢰도 | 중증도 | 실제 진단 매칭 | 판정 |
|----------|--------|--------|-------------|------|
| **심부전** | **74.1%** | MODERATE | ✅ Primary Dx | **TP** |
| **심방세동/조동** | **47.5%** | MODERATE | ✅ Afib with RVR | **TP** |
| 만성 허혈성 심질환 | 66.4% | MILD | — 학습레이블=1 | △ |
| 만성 신장질환 | 54.9% | MILD | — Golden에 없음 | 오탐 가능 |

- ECG Vitals: HR=83.4, regular rhythm
- Risk Level: **URGENT**
- 오탐: CKD 1개 (학습 데이터에서 HF 환자의 47%가 CKD 동반 → 레이블 상관관계)

**시연 발표:**

> "88세 남성, 호흡곤란으로 내원한 환자입니다.
> ECG AI가 심부전 74.1%를 검출했고, 실제 진단(Primary Dx)이 '급성 악화 이완기 심부전'으로 **정확히 일치**합니다.
> 심방세동도 47.5%로 감지했는데, Golden Dx에도 'Afib with RVR'이 기록되어 있어 **True Positive**입니다.
> 만성 신장질환 54.9%가 오탐 가능성이 있는데, 이는 학습 데이터에서 심부전 환자의 47%가 CKD를 동반하고 있어 모델이 함께 올리는 경향이 있기 때문입니다.
> 이 오탐은 **Lab 모달에서 Creatinine을 확인하면** 바로 걸러집니다."

**Lab 확인 항목:**

| 오탐 의심 | Lab 검사 | 확인 방법 |
|----------|---------|----------|
| CKD 54.9% | **Creatinine, eGFR** | Creatinine 1.4 → 경계값, eGFR로 확정 |

---

### Case 3: ECG 한계 → Lab 필수 — 급성 심근경색 45.1%

> **"NSTEMI 확진 환자인데 ECG 모델은 45.1%로 '의심' 수준입니다. 이것은 ECG의 한계가 아니라 NSTEMI의 특성입니다. ST 변화가 미미한 NSTEMI는 ECG만으로 확정할 수 없고, Troponin 혈액검사가 반드시 필요합니다."**

| 항목 | 내용 |
|------|------|
| 환자 | 79세 여성 (subject_id: 15238548, study_id: 48738546) |
| 주소 | NSTEMI, 불안정 협심증, 심부전, 갑상선기능저하증, COPD |
| 실제 진단 (MIMIC-IV 퇴원요약) | **NSTEMI** (Primary), CHF, Hypothyroidism, DM2, 사망 |

**EC2 실제 결과:**

| 감지 질환 | 신뢰도 | 중증도 | 실제 진단 매칭 | 판정 |
|----------|--------|--------|-------------|------|
| **심부전** | **67.3%** | MODERATE | ✅ CHF | **TP** |
| 만성 허혈성 심질환 | 65.1% | MILD | — CAD/CABG 이력 | 타당 |
| **급성 심근경색** | **45.1%** | **CRITICAL** | ✅ NSTEMI | **TP — Lab 확정 필요** |
| 급성 신부전 | 49.3% | MILD | ❌ Cr 1.1 정상 | **오탐** |
| 만성 신장질환 | 56.7% | MILD | ❌ Golden에 없음 | **오탐** |
| 호흡부전 | 38.2% | SEVERE | — COPD+폐부종 | 타당 |

- ECG Vitals: HR=94.3
- Risk Level: **CRITICAL** (acute_mi severity=critical)

**시연 발표:**

> "79세 여성, 호흡곤란과 턱까지 방사되는 흉통으로 내원했습니다.
> ECG AI가 급성 심근경색 45.1%를 검출했고, Risk Level이 **CRITICAL**로 올라갔습니다.
> 45.1%는 낮아 보이지만 Tier-1 임계값(0.30)을 넘겼고, 실제로 이 환자는 **NSTEMI 확진**(Troponin T 0.02→0.06 상승)입니다.
>
> **왜 45%밖에 안 나오나?** NSTEMI는 ST **상승**이 아닌 ST **하강** 또는 변화가 미미한 경우가 많아, ECG 파형만으로는 확정이 어렵습니다.
> 이것이 바로 **Lab 모달이 필요한 이유**입니다.
>
> 신장 관련 오탐(AKI 49.3%, CKD 56.7%)이 있는데, 실제 Creatinine은 1.1로 정상입니다.
> 학습 데이터에서 심부전 환자의 47%가 CKD를 동반하여 모델이 함께 올리는 현상입니다.
> Lab 모달이 Creatinine을 확인하면 즉시 걸러집니다."

**Lab 확인 항목:**

| 감지 항목 | Lab 검사 | 확인 방법 | 결과 |
|----------|---------|----------|------|
| acute_mi 45.1% | **Troponin I/T** | 연속 측정, 상승 추세 확인 | Troponin 0.02→0.06 → **NSTEMI 확진** |
| AKI 49.3% (오탐) | **Creatinine, BUN** | 수치 확인 | Cr 1.1 → 정상 → **오탐 확정, 제거** |
| CKD 56.7% (오탐) | **eGFR** | 수치 확인 | 정상 → **오탐 확정, 제거** |

**Bedrock Agent 흐름:**
```
ECG: acute_mi 45.1% (CRITICAL) → Bedrock 판단: "MI 의심, Troponin 확인 필요"
  → Lab 모달 호출 → Troponin 0.02→0.06 확인 → NSTEMI 확진
  → 최종 판단: "즉시 심장내과 협진 + PCI 팀 호출"

ECG: AKI 49.3% → Bedrock 판단: "신부전 의심, Creatinine 확인"
  → Lab 모달 호출 → Cr 1.1 확인 → 정상 → "AKI 해당 없음" → 오탐 제거
```

---

### Case 4: ECG 한계 → Lab 필수 — 고칼륨혈증 58.8%

> **"ECG에서 T파 텐팅(뾰족해짐) 패턴을 감지하여 고칼륨혈증 58.8%를 검출했습니다.
> 이 환자의 실제 진단에도 Hyperkalemia(K+=5.3)가 포함되어 있어 True Positive이지만,
> 최종 확정은 Lab 모달의 K+ 수치로 해야 합니다."**

| 항목 | 내용 |
|------|------|
| 환자 | 41세 여성 (subject_id: 14866589, study_id: 40812977) |
| 주소 | T1DM, NSTEMI 이력, AKI, 고칼륨혈증, 저나트륨혈증 |
| 실제 진단 (MIMIC-IV 퇴원요약) | Viral URI (Primary), **AKI, Hyperkalemia (K+=5.3)**, Hyponatremia, DM1 |

**EC2 실제 결과:**

| 감지 질환 | 신뢰도 | 중증도 | 실제 진단 매칭 | 판정 |
|----------|--------|--------|-------------|------|
| 만성 신장질환 | 89.4% | MILD | — Golden에 CKD 없음 | 오탐 가능 (AKI 환자라 연관) |
| 심부전 | 66.8% | MODERATE | — 학습레이블=1 (EF 40%) | △ |
| **고칼륨혈증** | **58.8%** | **SEVERE** | ✅ Hyperkalemia | **TP** |
| 심부전(세부) | 51.6% | MODERATE | — 학습레이블=1 | △ |
| **급성 신부전** | **31.2%** | MILD | ✅ AKI | **TP** |
...
> "41세 여성, 기침과 다리 통증으로 내원한 환자입니다.
> Primary Dx는 바이러스 감기지만, Secondary에 **고칼륨혈증(K+=5.3)과 AKI**가 있습니다.
> ECG AI가 고칼륨혈증 58.8%를 검출했습니다. 고칼륨혈증은 T파가 뾰족해지는 **ECG 특이 소견**이 있어 파형에서 감지 가능합니다.
>
> 다만 ECG만으로는 K+ 수치를 알 수 없으므로, **Lab 모달에서 K+ 수치를 직접 측정하여 확정**해야 합니다.
> 실제로 이 환자의 K+=5.3으로 고칼륨혈증이 맞습니다.
>
> 흥미로운 점은, 학습 레이블에서 이 ECG에 hyperkalemia=0으로 되어있는데도 모델이 58.8%로 잡았다는 것입니다.
> 이는 모델이 레이블이 아닌 **실제 ECG 파형의 T파 패턴을 학습**했다는 증거입니다."

**Lab 확인 항목:**

| 감지 항목 | Lab 검사 | 확인 방법 | 결과 |
|----------|---------|----------|------|
| hyperkalemia 58.8% | **혈중 K+** | 수치 확인 (정상 3.5~5.0) | K+=5.3 → **고칼륨혈증 확진** |
| AKI 31.2% | **Creatinine, BUN** | 수치 확인 | → AKI 여부 확정 |
| CKD 89.4% (오탐 가능) | **eGFR** | 수치 확인 | → CKD 여부 확정 |

**Bedrock Agent 흐름:**
```
ECG: hyperkalemia 58.8% (SEVERE) → Bedrock 판단: "K+ 이상 의심"
  → Lab 모달 호출 → K+=5.3 확인 → 고칼륨혈증 확진
  → 최종 판단: "칼륨 교정, 심전도 모니터링, 신장내과 협진"
```

---

### Case 5: ECG 경고 → 멀티모달 확정 — 패혈증 30.1% + 호흡부전 36.1%

> **"외과 환자(소장 폐색)인데 ECG가 빈맥(HR=124.9)과 함께 패혈증 30.1%, 호흡부전 36.1%를 감지했습니다.
> ECG만으로는 확정할 수 없지만, Bedrock Agent가 이 수치를 보고 Blood 모달(혈액배양)과 Chest 모달(흉부 X-ray)을 호출합니다."**

| 항목 | 내용 |
|------|------|
| 환자 | 83세 남성 (subject_id: 15968916, study_id: 44082311) |
| 주소 | 고위급 소장 폐색, 감돈 탈장, 발열, COPD |
| 실제 진단 (MIMIC-IV 퇴원요약) | 소장 폐색 (Primary), **Post-op atrial tachycardia** (패혈증 합병) |

**EC2 실제 결과:**

| 감지 질환 | 신뢰도 | 중증도 | 실제 진단 매칭 | 다음 모달 |
|----------|--------|--------|-------------|----------|
| 급성 신부전 | 42.8% | MILD | — | **Blood** (Creatinine/BUN) |
| **심방세동/조동** | **42.6%** | MODERATE | ✅ atrial tachycardia | — |
| **호흡부전** | **36.1%** | **SEVERE** | — COPD 환자 | **Chest** (흉부 X-ray) |
| **패혈증** | **30.1%** | **SEVERE** | — 합병증 | **Blood** (혈액배양+젖산) |
...
> "83세 남성, 소장 폐색으로 수술이 필요한 외과 환자입니다.
> 주소만 보면 '장 문제'인데, ECG AI가 **전신 위험 신호**를 잡아냈습니다.
>
> 패혈증 30.1%는 Tier-1 임계값(0.30)에 딱 걸립니다.
> 사망률 28.1%인 질환이므로 30%만 넘어도 놓치면 안 됩니다.
> HR=124.9 빈맥은 패혈증의 교감신경 항진 반응과 일치합니다.
>
> ECG만으로는 패혈증을 확정할 수 없습니다. 하지만 이 수치를 보고 Bedrock Agent가:
> - 패혈증 30.1% → **Blood 모달 호출** (혈액배양 + 젖산 검사)
> - 호흡부전 36.1% → **Chest 모달 호출** (흉부 X-ray)
> - 급성 신부전 42.8% → **Blood 모달 호출** (Creatinine/BUN)
>
> 이것이 멀티모달 시스템의 핵심 가치입니다.
> ECG가 감지 → 중앙(Bedrock Agent)이 판단 → 다른 모달이 확정."

**Lab/CXR 확인 항목:**

| 감지 항목 | 담당 모달 | 검사 | 확인 방법 |
|----------|---------|------|----------|
| sepsis 30.1% | **Blood** | 혈액배양 + **젖산(Lactate)** | Lactate > 2 → 패혈증 확진 |
| resp_failure 36.1% | **Chest** | **흉부 X-ray** | 폐침윤, 흉수, 무기폐 확인 |
| AKI 42.8% | **Blood** | **Creatinine, BUN** | Cr 상승 → AKI 확진 |

**임상 근거 — "장폐색인데 왜 패혈증/호흡부전이 나오나?"**
...
ECG는 장폐색 자체를 검출한 것이 아니라, 장폐색으로 인해 몸 전체에서 일어나고 있는 **전신 반응**(패혈증, 호흡부전, 신부전)을 심전도 파형에서 감지한 것입니다.

---

## ECG 모달 학습 평가 — 5개 케이스 종합

### 정량 평가

| Case | TP | FP | FN | 학습 평가 |
|:---:|:---:|:---:|:---:|---|
| 1 (Afib+HF) | 3 | **0** | 2 | **우수** — 핵심 질환 정확, 오탐 없음 |
| 2 (HF Primary) | 2 | 1 | 1 | **우수** — Primary Dx 정확 |
| 3 (NSTEMI) | 2 | 2 | 2 | **양호** — MI 의심→Lab 확정 흐름 |
| 4 (HK) | 1 | 1 | 0 | **우수** — 레이블=0인데도 파형 패턴 감지 |
| 5 (Sepsis) | 0 | 2 | 0 | **양호** — 전신 위험 경계 감지 |

### 학습이 잘 된 질환

| 질환 | 5개 케이스 결과 | AUROC | 평가 |
|------|---------------|-------|------|
| **Afib** | 94%, 47.5%, 42.6% — 3개 케이스에서 모두 감지 | 0.903 | **최우수** |
| **Heart Failure** | 74.1%, 70.4%, 67.3%, 66.8% — 4개 케이스에서 감지 | 0.897 | **최우수** |
| **Hyperkalemia** | 58.8% — 학습레이블=0인데도 ECG 파형 패턴 자체를 학습 | 0.820 | **우수** |
| **acute_mi** | 45.1% — NSTEMI 의심 수준, Tier-1 임계값으로 감지 | 0.846 | **우수** |
| **Sepsis** | 30.1% — 경계 감지, ECG Vitals(빈맥)과 일치 | 0.869 | **양호** |

### 공통 오탐 패턴과 해결

| 오탐 | 원인 | 해결 |
|------|------|------|
| AKI/CKD 반복 감지 | 학습 데이터에서 HF 환자의 47%가 CKD 동반 → 레이블 상관관계 학습 | **Lab 모달이 Creatinine/eGFR로 즉시 확인하여 제거** |

### ECG 모달의 역할 정리

```
ECG 모달의 가치 =
  ① 확정 가능한 건 바로 확정 (Afib 94%, HF 74%, HK 59%)
  ② 의심 신호를 잡아서 Lab으로 연결 (MI 45% → Troponin, HK 59% → K+)
  ③ 사망률 높은 질환은 낮은 확률이라도 놓치지 않음 (Sepsis 30.1%)
  ④ 오탐은 Lab 모달이 걸러줌 (AKI/CKD → Creatinine 확인)
```
...
```
Case 1, 2:  ECG 파형 → AI 분석 → 질환 확정 (Afib 94%, HF 74%)
  "심전도만으로 이 질환을 정확하게 잡아냅니다"

Case 3, 4:  ECG 파형 → AI 의심 → Lab 확정 필요 (MI 45%, HK 59%)
  "심전도가 의심하고, 혈액검사가 확정합니다"

Case 5:     ECG 파형 → AI 경고 → Blood/Chest 호출 (Sepsis 30%)
